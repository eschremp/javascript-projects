...
function reverseCharacters(chars) {
    if (typeof(chars) === 'string') {
        return(chars.split('').toReversed().join(''));
} else {
    return Number(String(chars).split('').toReversed().join(''));
}
}
...
for (let i = 0; i < arrayTest.length; i++) {
    reverseItemArr.push(reverseCharacters(arrayTest[i]));
}
console.log(reverseItemArr);*/
...
let length = 5;
let height;
console.log(`The area is ${areaOfRectangle(length, height)} cm^2.`);